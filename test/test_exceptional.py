...
def test_comprehensive_exceptions(test_obj):
    ...
    try:
        ...
        if check_class_exists(module_obj, "Member"):
            try:
                member_cls = getattr(module_obj, "Member")
        ...
        if check_class_exists(module_obj, "Library"):
            try:
                library = safely_create_instance(module_obj, "Library", "Search Library", "Search St")
        ...
        if check_class_exists(module_obj, "Book") and check_class_exists(module_obj, "Member"):
            try:
                # Test borrowing unavailable book
                checked_out_book = safely_create_instance(module_obj, "Book", "B001", "Unavailable Book", "Author", "Fiction", 2020, False)
                member = safely_create_instance(module_obj, "Member", "M001", "Test Member", "test@example.com")
                ...
                if checked_out_book is not None and member is not None:
                    ...
                    if check_function_exists(member, "borrow_book"):
                        books = []
                        for i in range(2, 6):
                            book = safely_create_instance(module_obj, "Book", f"B{i}", f"Book {i}", "Author", "Fiction", 2020)
                            if book is not None:
                                books.append(book)
        ...
        if check_class_exists(module_obj, "Library") and check_class_exists(module_obj, "Book") and check_class_exists(module_obj, "Member"):
            try:
                lib = safely_create_instance(module_obj, "Library", "Test Library", "Test St")
                book = safely_create_instance(module_obj, "Book", "B101", "Test Book", "Author", "Fiction", 2020)
                member = safely_create_instance(module_obj, "Member", "M101", "Test Member", "test@example.com")
                ...
                if lib is not None and book is not None and member is not None:
                    ...
                    new_library = safely_create_instance(module_obj, "Library", "Operations Library", "Operations St")
                    if new_library is not None:
                        ...
                        book2 = safely_create_instance(module_obj, "Book", "B011", "Exists", "Author", "Fiction", 2020)
                        if book2 is not None:
                            ...
                            member3 = safely_create_instance(module_obj, "Member", "M007", "Exists", "exists@example.com")
        ...
        if check_class_exists(module_obj, "FictionBook"):
            try:
                # Test FictionBook with invalid parameters
                fiction_cls = getattr(module_obj, "FictionBook")
                # Most validation should be in base Book class, so just test creation
                fiction_book = safely_create_instance(module_obj, "FictionBook", "FB001", "Fiction Test", "Author", "Fiction", 2020, "Novel")
        ...
        if check_class_exists(module_obj, "NonFictionBook"):
            try:
                # Test NonFictionBook with invalid parameters
                nonfiction_cls = getattr(module_obj, "NonFictionBook")
                nonfiction_book = safely_create_instance(module_obj, "NonFictionBook", "NF001", "NonFiction Test", "Author", "NonFiction", 2020, "Science")
        ...
        if errors:
            ...
            test_obj.yakshaAssert("test_comprehensive_exceptions", True, "exceptional")
            
    except Exception as e:
        test_obj.yakshaAssert("test_comprehensive_exceptions", False, "exceptional")
        pytest.fail(f"Comprehensive exceptions test failed: {str(e)}")
